...
def string_is_safe(s, filename=False):
    ...
    for c in s:
        if c < 0x20 or c > 0x7F:
            return False
        if filename and not ((c >= 0x41 and c <= 0x5A) or
...
_date_re = re.compile(b'^\d\d\d\d-\d\d-\d\d$')
...
def random_passphrase(length):
    pw = base64.urlsafe_b64encode(os.urandom(length))[:length]
    pw = pw.decode('utf-8')
    return pw
...
def unbind_passphrase(bound_passphrase):
    ...
    """
    while bound_passphrase.startswith((b'{', b'[')):
